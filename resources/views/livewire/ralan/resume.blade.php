<div @if($isCollapsed) class="card card-info collapsed-card" @else class="card card-info" @endif>
    ...
    <!-- Modal Terapi -->
    <div class="modal fade" id="terapiModal" tabindex="-1" role="dialog" aria-labelledby="terapiModalTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                <div class="table-responsive">
                    @forelse ($listTerapi as $i => $item)  
                        <div class="custom-control custom-checkbox">
                            <input id="terapiCheck-{{ $i }}" wire:key='terapiCheck-{{ $i }}' class="custom-control-input" wire:model.defer='checkTerapi' value="{{ $item->nama_brng }} : {{ $item->jml }} {{ $item->kode_sat }}" type="checkbox" name="terapiCheck[]">
